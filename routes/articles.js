...
router.get('/', auth.isAuthorized, async (req, res) => {
    ...
    res.render('articles', {
        ...
        title_suffix: "",
        articles: (await db(`users/${req.session.user}`).select('articles', pagger(currentPage, MaxPerPage, filterOnArticleReadState(state)))).reverse(),
...
router.get('/tagged/:tag', auth.isAuthorized, async (req, res) => {
    ...
    res.render('articles', {
        ...
        articles: (await db(`users/${req.session.user}`).select('articles', filter)).filter(pagger(currentPage, MaxPerPage)).reverse(),
        tags: await db(`users/${req.session.user}`).select('tags', _ => true),
        currentPage: currentPage,
        totalPages: Math.ceil(total / MaxPerPage),
    })
